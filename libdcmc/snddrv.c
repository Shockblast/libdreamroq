...
libdreamroq_snd_stream_hnd_t shnd;
...
int snddrv_volume_up() {
    ...
    if( snddrv_vol <= 245 ) {
        snddrv_vol += 10;
  	    libdreamroq_snd_stream_volume(shnd, snddrv_vol);
    }
    return snddrv_vol;
}
...
static void *snddrv_callback(libdreamroq_snd_stream_hnd_t hnd, int len, int * actual) {
...
static void *snddrv_thread(void *param) {
	...
	shnd = libdreamroq_snd_stream_alloc(snddrv_callback, SND_STREAM_BUFFER_MAX/4);

    libdreamroq_snd_stream_start(shnd, snddrv.rate, snddrv.channels-1);
	...
	while( snddrv.drv_status != SNDDRV_STATUS_DONE && snddrv.drv_status != SNDDRV_STATUS_ERROR ) {
		...
		libdreamroq_snd_stream_poll(shnd);
		thd_sleep(20);

	}
    snddrv.drv_status = SNDDRV_STATUS_NULL;

    libdreamroq_snd_stream_destroy(shnd);
	libdreamroq_snd_stream_shutdown();
